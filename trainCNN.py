...
def main():
    ...
    parser.add_argument("-a", "--architecture", type=str, choices=['CCFF'], default="ccfff", help="choose CNN architecture")
    ...
    # Save trained model
    if args.save_model:
        # model filename format: <dataset>_<architecture>_<num_epochs>_<batch_size>_<loss>_model.npz
        model_filename = "%s_%s_%d_%d_%s_model.npz" % \
                         (args.dataset, args.architecture, args.num_epochs, args.batch_size, args.loss)
        create_trained_models_dir(network, model_filename)
